...
function ApartForRent() {
  ...
  useEffect(() => {
    const result = apartForRentList.filter((apartForRentList) => apartForRentList.apartment_code.toLowerCase().match(search.toLowerCase())
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: 'price',
      cell: (row) => (
        <div className="py-1 px-2 font-medium text-md">
      ...
      cell: (row) => (
        <span className="text-center">
          <button
            ...
            className="text-white py-1 px-2 mb-2 capitalize rounded-2xl text-md mr-1"
            onClick={() => {
              if (window.confirm('Are you sure to delete this item?')) deleteApartForRent(row.id_apartment);
            }}
          >
            Delete
          </button>
